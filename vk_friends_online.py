...
def get_user_login_and_password_from_cmd():
    print('Программа показывает кто из ваших друзей '
          'Вконтакте онлайн в данный момент')
...
def get_online_vk_friends(vk_login, vk_password, scope=None):
    ...
    online_friends_list = vk_api.users.get(user_ids=online_friends_ids_list,
                                           fields='online')
    return online_friends_list


def output_friends_to_console(online_friends_list):
    print('В данный момент {} ваших друзей '
    ...
    for index, friend_data in enumerate(online_friends_list):
        ...
        print('{4:<3} {0:10} {1:20} '
              'https://vk.com/id{2:<15} {3}'.format(friend_data['first_name'],
                                                    friend_data['last_name'],
                                                    friend_data['uid'],
                                                    friend_from_mobile_text,
                                                    index + 1))
    print('\n')


def configurate_cmd_parser():
    parser_description = ('\nСкрипт показывает кто из ваших друзей '
                          'вконтакте онлайн в данный момент.\n')
...
if __name__ == '__main__':
    ...
    try:
        friends_online = get_online_vk_friends(vk_login,
                                               vk_password,
                                               scope='friends, users')
